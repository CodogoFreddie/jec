...
const Task = ({description, tags, due, wait, project, ...rest}) => (
  <Card>
    ...
    <Card.Content>
      ...
      <Card.Description>{due && 'due ' + formatDate(due)}</Card.Description>
      <Card.Description>{wait && 'wait ' + formatDate(wait)}</Card.Description>
    ...
    return this.props.__distributeStatus === 'READY' ? (
      <Segment>
        ...
        <Card.Group centered stackable itemsPerRow={1}>
          {R.sortBy(({score}) => -score, collateAllObjects(this.props)).map(
