...
export default ({ ids, uuids, props, tags, description, }) => {
	const tests = [
		...
		( description.length && ( ({ description: objectDesc, }) => objectDesc.includes(description) ) ),
		( tags.length && filterByTags(tags) ),
		( props.project && ( ({ project = "", }) => project.includes(props.project) ) ),
	].filter(Boolean);

	const filterer = tests.length
		? R.filter(R.anyPass(tests))
		: R.identity;
