...
const testRecur = test => R.pipe(R.prop("op"), R.test(test));

const addUnit = ({ op, n, }) => df[op](n);

const startOfUnit = ({ op, }) =>
	R.pipe(df[op.replace("startOf", "endOf")], df.addMilliseconds(1), df[op]);

const endOfUnit = ({ op, }) => R.pipe(df[op], df.addMilliseconds(1), df[op]);

export const generateRecurrenceFunction = R.cond([
	[testRecur(/^add/), addUnit,],
	[testRecur(/^startOf/), startOfUnit,],
	[testRecur(/^endOf/), endOfUnit,],
]);
